...
use tokio_rustls::rustls::{
    AllowAnyAuthenticatedClient, RootCertStore, ServerConfig, Session, TLSError,
};
...
impl TlsConfigBuilder {
    ...
    pub fn client_ca_path(mut self, path: impl AsRef<Path>) -> Self {
        self.ca = Box::new(LazyFile {
            path: path.as_ref().into(),
            file: None,
        });
        self
    ...
    pub fn client_ca(mut self, ca: &[u8]) -> Self {
        self.ca = Box::new(Cursor::new(Vec::from(ca)));
        self
    }
    ...
    pub fn build(mut self) -> Result<ServerConfig, TlsConfigError> {
        ...
        let mut ca_rdr = BufReader::new(self.ca);
...
impl TlsStream {
    fn new(stream: AddrStream, config: Arc<ServerConfig>) -> TlsStream {
        // let accept = tokio_rustls::TlsAcceptor::from(config).accept_with(stream, |s| {
        //     let new_sess = ServerSession::new(&config);
        //     *s = new_sess;
        // });
        // TlsStream {
